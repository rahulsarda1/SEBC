...
copy public key to all machines /root/.ssh/id_rsa.pub to vi /root/.ssh/authorized_keys
...
## Add the following line GRUB_CMDLINE_LINUX options in the /etc/default/grub file
```sh
...
TX errors 0  dropped 0 overruns 0  carrier 0  collisions 0
```
# yum repositories prerequisites needed to install Cloudera
```sh
yum -y install bind*
yum -y install wget
yum -y install nscd
...
chkconfig ntpd on
```
# Current not installed but can be useful
```sh
yum -y install perl
...
# Setup MariaDb
Seperate documentation refer file 2_replica_working.md
...
# Install JDBC Connector on all nodes
...
```
# Run Cloudera manager wizard
Enter all the host details
No SIngle Host
3 Zookepper
